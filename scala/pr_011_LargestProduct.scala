...
val matrix = Vector(
  ...
  def printMatrix(v: Vector[Vector[Int]]): Unit = {
    println(v.map(x => x.map(y => " %02d" format y).mkString).mkString("\n"))
  }

  def transp(v: Vector[Vector[Int]]): Vector[Vector[Int]] = {
    def column(c: Int): Vector[Int] = {
      v.map(x => x(c))
    }
    (0 until v.size).map(column).toVector
  }

  val n = matrix.size

  def transpVert(v: Vector[Vector[Int]]): Vector[Vector[Int]] = {
    def columnVert(c: Int): Vector[Int] = {
      val m = c % n
      val cNum = if (c == m) 0 until m else m until n
      val coord = cNum zip cNum.reverse
      coord.map({ case (line, col) => v(line)(col)}).toVector
    }
    0 until 2 * n  map columnVert toVector
  }

  def prodList(l: List[Vector[Vector[Int]]]):Int = {
    def prod(vect:Vector[Int]):Vector[Int]={
      (0 until vect.size-4).map(x=>vect.slice(x,x+4)).map(y=>y.foldLeft(1)((i,j)=>i*j)).toVector
    }
    l.flatMap(x=>x.filter(y=>y.size>3).map(prod)).flatten.max
  }

  def main(args: Array[String]) {
    val transp1: Vector[Vector[Int]] = transp(matrix)
    println(prodList(List(matrix,transp1,transpVert(matrix),transpVert(transp1))))
    //printMatrix(transpVert(matrix))48477312
    /*printMatrix(matrix) // 48477312
    println()
    printMatrix(transp(matrix))*/
  }
}
